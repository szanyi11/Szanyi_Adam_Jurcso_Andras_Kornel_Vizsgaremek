...
async function redrawUsers() {
    ...
    for (const u of window.users) {
        ...
        td2.innerText = u.felhasznalo_nev
        tr.appendChild(td2)
...
function usersearch(event) {
    ...
    window.users = window.allusers.filter(
        item => item.uid.toUpperCase().includes(f) || item.felhasznalo_nev.toUpperCase().includes(f)
    )
    currentuser = window.users[0]
    redrawUsers()
}
...
function modifyuser() {
    ...
    for (let i = 1; i < trs.length; i++) {
        if (trs[i].children[0].innerText === currentuser.uid) {
            ...
            input.setAttribute('value', currentuser.felhasznalo_nev)
            input.id = 'felhasznalo_nev'
...
function saveUser() {
    ...
    Service.saveUser(new User(
        uid,
        document.getElementById('felhasznalo_nev').value,
        pwd,
        document.getElementById('activeuser').checked ? 1 : 0
    ))
